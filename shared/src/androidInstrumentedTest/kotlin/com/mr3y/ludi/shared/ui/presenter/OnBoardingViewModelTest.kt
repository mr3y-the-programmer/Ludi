...
class OnBoardingViewModelTest {
    ...
    fun user_updates_preferences_new_preferences_are_saved_and_state_is_observable() = testScope.runTest {
        ...
        val followedNewsDataSource = NewsDataSource("Game spot", iconResName = "game_spot_logo.xml", drawableId = 1592037, type = Source.GameSpot)
        // make sure the function is idempotent
        repeat(2) { sut.followNewsDataSource(followedNewsDataSource) }
        repeat(2) { sut.followNewsDataSource(followedNewsDataSource.copy(name = "foo bar", drawableId = 1293137)) }
